...
pub struct MmioDevice {
    ...
    irqchip: Arc<dyn IrqChip>,
    avail_evts: Vec<EventFd>,
}

impl MmioDevice {
    /// Constructs a new MMIO transport for the given virtio device.
    pub fn new(mem: GuestMemory, device: Box<dyn VirtioDevice>,
        irqchip: Arc<dyn IrqChip>) -> Result<MmioDevice> {
        let mut queue_evts = Vec::new();
        let mut avail_evts = Vec::new();
        for _ in device.queue_max_sizes().iter() {
            let evt_fd = EventFd::new()?;
            queue_evts.push(evt_fd.try_clone()?);
            avail_evts.push(evt_fd);
        }
        let queues = device
            .queue_max_sizes()
            .iter()
