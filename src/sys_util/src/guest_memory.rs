...
impl GuestMemory {
    ...
    */
    ...
    {
        for (index, region) in self.regions.read().unwrap().iter().enumerate() {
            cb(
                index,
                region.guest_base,
                region.mapping.size(),
    ...
    */
    ...
    {
        let page_addr = GuestAddress(MEM_START as usize);
        self.regions.read().unwrap()
            .get(&page_addr)
            .map_or(
    ...
    fn get_slice(&self, offset: usize, count: usize) -> VolatileMemoryResult<VolatileSlice> {
        for region in self.regions.read().unwrap().iter() {
            if offset >= region.guest_base.0 && offset < region_end(region).0 {
                ...
                return Ok(unsafe {
                    VolatileSlice::new(
                        ((offset - region.guest_base.0) as usize + offset) as *mut _, 
                        count)
                });
            }
        }
